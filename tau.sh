...
tau measurement create $MEASNAME --metrics="TIME,PAPI_FP_INS,PAPI_TOT_INS" --compiler-inst fallback --sample F --source-inst automatic
tau select $MEASNAME

# flops
MEASNAME=flops
tau measurement create $MEASNAME --metrics="TIME,PAPI_FP_OPS" --compiler-inst fallback  --sample F --source-inst automatic
tau select $MEASNAME
...
# uops
MEASNAME=uops
tau measurement create $MEASNAME --metrics="TIME,PAPI_NATIVE:UOPS_RETIRED:SCALAR_SIMD,PAPI_NATIVE:UOPS_RETIRED:PACKED_SIMD,PAPI_NATIVE:UOPS_RETIRED:ALL" --compiler-inst fallback --sample F --source-inst automatic
tau select $MEASNAME

# cpi
MEASNAME=cpi
tau measurement create $MEASNAME --metrics="TIME,PAPI_TOT_CYC,PAPI_TOT_INS" --compiler-inst fallback --sample F --source-inst automatic
tau select $MEASNAME

# stalls
MEASNAME=stalls
tau measurement create $MEASNAME --metrics="TIME,PAPI_TOT_CYC,PAPI_STL_ICY" --compiler-inst fallback --sample F --source-inst automatic
tau select $MEASNAME
